...
public final class HashSemiJoinRecordCursorFactory extends AbstractJoinRecordCursorFactory {

    private final HashSemiJoinRecordCursor cursor;
    private final RecordSink masterSink;
    private final RecordSink slaveKeySink;

    public HashSemiJoinRecordCursorFactory(
        ...
        try {
            ...
            this.cursor = new HashSemiJoinRecordCursor(configuration, joinColumnTypes);
        } catch (Throwable th) {
            close();
            throw th;
        }
    ...
    public void toPlan(PlanSink sink) {
        sink.type("Semi Hash Join Light");
    ...
    private static void populateSlaveHashMap(
    ...
    private class HashSemiJoinRecordCursor implements NoRandomAccessRecordCursor {
        ...
        public HashSemiJoinRecordCursor(CairoConfiguration configuration, ColumnTypes joinColumnTypes) {
            try {
                isOpen = true;
                joinKeyMap = MapFactory.createUnorderedMap(configuration, joinColumnTypes, null);
            } catch (Throwable th) {
                close();
                throw th;
            }
        ...
        private void buildMapOfSlaveRecords() {
            if (!isMapBuilt) {
                populateSlaveHashMap(circuitBreaker, slaveCursor, joinKeyMap, slaveKeySink);
                isMapBuilt = true;
            }
        }
