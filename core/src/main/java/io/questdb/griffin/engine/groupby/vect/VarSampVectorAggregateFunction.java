...
public class VarSampVectorAggregateFunction extends DoubleFunction implements VectorAggregateFunction {
    ...
    public VarSampVectorAggregateFunction(int keyKind, int columnIndex, int workerCount) {
    ...
    public void aggregate(long address, long frameRowCount, int workerId) {
        if (address != 0) {
            final double value = Vect.varSamp(address, frameRowCount);
            if (value == value) {
